...
import {
  ...
  deleteDepartment,
  searchDepartments,
} from '../../../../../app/modules/entities/department/department.sagas'
...
test('search success path', () => {
  const response = FixtureAPI.searchDepartments()
  const step = stepper(searchDepartments(FixtureAPI, '*'))
  // Step 1: Hit the api
  step()
  // Step 2: Successful return and data!
  expect(step(response)).toEqual(put(DepartmentActions.departmentSearchSuccess([{ id: 1 }, { id: 2 }])))
})
...
test('search failure path', () => {
  ...
  step()
  // Step 2: Failed response.
  expect(step(response)).toEqual(put(DepartmentActions.departmentSearchFailure()))
})
