...
import {
  ...
  deleteJobHistory,
  searchJobHistories,
} from '../../../../../app/modules/entities/job-history/job-history.sagas'
...
test('search success path', () => {
  const response = FixtureAPI.searchJobHistories()
  const step = stepper(searchJobHistories(FixtureAPI, '*'))
  // Step 1: Hit the api
  step()
  // Step 2: Successful return and data!
  expect(step(response)).toEqual(put(JobHistoryActions.jobHistorySearchSuccess([{ id: 1 }, { id: 2 }])))
})
...
test('search failure path', () => {
  ...
  step()
  // Step 2: Failed response.
  expect(step(response)).toEqual(put(JobHistoryActions.jobHistorySearchFailure()))
})
