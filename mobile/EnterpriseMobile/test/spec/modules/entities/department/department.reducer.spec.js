...
test('attempt searching a department', () => {
  const state = reducer(INITIAL_STATE, Actions.departmentSearchRequest(1))
...
test('success searching a department', () => {
  const state = reducer(INITIAL_STATE, Actions.departmentSearchSuccess({ id: 1 }))

  expect(state.searching).toBe(false)
  expect(state.errorSearching).toBe(null)
  expect(state.departments).toEqual({ id: 1 })
})
...
test('failure searching a department', () => {
  const state = reducer(INITIAL_STATE, Actions.departmentSearchFailure({ error: 'Not found' }))
