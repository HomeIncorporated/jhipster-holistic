...
export function* searchJobs(api, action) {
  const { query } = action
  // make the call to the api
  const apiCall = call(api.searchJobs, query)
  const response = yield call(callApi, apiCall)

  // success?
  if (response.ok) {
    yield put(JobActions.jobSearchSuccess(response.data))
  } else {
    yield put(JobActions.jobSearchFailure(response.data))
  }
}
