...
import { getRegion, getRegions, updateRegion, deleteRegion, searchRegions } from '../../../../../app/modules/entities/region/region.sagas'
...
test('search success path', () => {
  const response = FixtureAPI.searchRegions()
  const step = stepper(searchRegions(FixtureAPI, '*'))
  // Step 1: Hit the api
  step()
  // Step 2: Successful return and data!
  expect(step(response)).toEqual(put(RegionActions.regionSearchSuccess([{ id: 1 }, { id: 2 }])))
})
...
test('search failure path', () => {
  ...
  step()
  // Step 2: Failed response.
  expect(step(response)).toEqual(put(RegionActions.regionSearchFailure()))
})
