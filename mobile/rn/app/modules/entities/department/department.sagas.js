...
export function* searchDepartments(api, action) {
  const { query } = action
  // make the call to the api
  const apiCall = call(api.searchDepartments, query)
  const response = yield call(callApi, apiCall)

  // success?
  if (response.ok) {
    yield put(DepartmentActions.departmentSearchSuccess(response.data))
  } else {
    yield put(DepartmentActions.departmentSearchFailure(response.data))
  }
}
