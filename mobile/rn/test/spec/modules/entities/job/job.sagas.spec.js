...
import { getJob, getJobs, updateJob, deleteJob, searchJobs } from '../../../../../app/modules/entities/job/job.sagas'
...
test('search success path', () => {
  const response = FixtureAPI.searchJobs()
  const step = stepper(searchJobs(FixtureAPI, '*'))
  // Step 1: Hit the api
  step()
  // Step 2: Successful return and data!
  expect(step(response)).toEqual(put(JobActions.jobSearchSuccess([{ id: 1 }, { id: 2 }])))
})
...
test('search failure path', () => {
  ...
  step()
  // Step 2: Failed response.
  expect(step(response)).toEqual(put(JobActions.jobSearchFailure()))
})
