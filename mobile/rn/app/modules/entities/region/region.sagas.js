...
export function* searchRegions(api, action) {
  const { query } = action
  // make the call to the api
  const apiCall = call(api.searchRegions, query)
  const response = yield call(callApi, apiCall)

  // success?
  if (response.ok) {
    yield put(RegionActions.regionSearchSuccess(response.data))
  } else {
    yield put(RegionActions.regionSearchFailure(response.data))
  }
}
