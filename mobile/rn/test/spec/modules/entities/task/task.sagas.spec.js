...
import { getTask, getTasks, updateTask, deleteTask, searchTasks } from '../../../../../app/modules/entities/task/task.sagas'
...
test('search success path', () => {
  const response = FixtureAPI.searchTasks()
  const step = stepper(searchTasks(FixtureAPI, '*'))
  // Step 1: Hit the api
  step()
  // Step 2: Successful return and data!
  expect(step(response)).toEqual(put(TaskActions.taskSearchSuccess([{ id: 1 }, { id: 2 }])))
})
...
test('search failure path', () => {
  ...
  step()
  // Step 2: Failed response.
  expect(step(response)).toEqual(put(TaskActions.taskSearchFailure()))
})
