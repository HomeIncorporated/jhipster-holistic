...
test('attempt searching a region', () => {
  const state = reducer(INITIAL_STATE, Actions.regionSearchRequest(1))
...
test('success searching a region', () => {
  const state = reducer(INITIAL_STATE, Actions.regionSearchSuccess({ id: 1 }))

  expect(state.searching).toBe(false)
  expect(state.errorSearching).toBe(null)
  expect(state.regions).toEqual({ id: 1 })
})
...
test('failure searching a region', () => {
  const state = reducer(INITIAL_STATE, Actions.regionSearchFailure({ error: 'Not found' }))
