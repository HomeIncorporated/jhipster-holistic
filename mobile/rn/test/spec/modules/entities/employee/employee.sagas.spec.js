...
import {
  ...
  deleteEmployee,
  searchEmployees,
} from '../../../../../app/modules/entities/employee/employee.sagas'
...
test('search success path', () => {
  const response = FixtureAPI.searchEmployees()
  const step = stepper(searchEmployees(FixtureAPI, '*'))
  // Step 1: Hit the api
  step()
  // Step 2: Successful return and data!
  expect(step(response)).toEqual(put(EmployeeActions.employeeSearchSuccess([{ id: 1 }, { id: 2 }])))
})
...
test('search failure path', () => {
  ...
  step()
  // Step 2: Failed response.
  expect(step(response)).toEqual(put(EmployeeActions.employeeSearchFailure()))
})
